...
import com.cariochi.objecto.Generate.BigDecimals.Scale;
...
public class BigDecimalScale implements ConfigFunctionFactory<Scale> {

    @Override
    public ConfigFunction createSettings(Scale annotation, ReflectoMethod method) {
        final BiFunction<ObjectoConfig, Context, ObjectoConfig> function = (config, context) -> config.withBigDecimals(
                config.bigDecimals().withScale(annotation.value()));
